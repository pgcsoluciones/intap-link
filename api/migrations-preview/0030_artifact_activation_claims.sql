-- B2B.2: Preview copy of the atomic activation claim marker and triggers.
-- Keep this migration separate from the Production sequence.
-- Preview sequence: 0030 because Preview already uses 0029 for intents.
-- PR #76 has divergent numbering; do not merge its sequence here yet.
...
BEGIN
  UPDATE intap_artifacts
   ...
   WHERE id = (SELECT i.artifact_id FROM artifact_activation_intents i WHERE i.intent_hash = NEW.intent_hash)
  ...
  UPDATE artifact_activation_codes
   ...
   WHERE id = (SELECT i.activation_code_id FROM artifact_activation_intents i WHERE i.intent_hash = NEW.intent_hash)
     AND status = 'active'
  ...
  SELECT CASE WHEN NOT EXISTS (
    ...
    JOIN intap_artifacts a ON a.id = i.artifact_id
    WHERE i.intent_hash = NEW.intent_hash
      AND i.status = 'consumed' AND i.consumed_at = NEW.claim_at
      AND ac.status = 'used' AND ac.used_at = NEW.claim_at
      AND a.owner_user_id = NEW.user_id AND a.status = 'activated'
      AND a.activated_at = NEW.claim_at
