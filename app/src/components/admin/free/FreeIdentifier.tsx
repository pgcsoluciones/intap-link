    ...
    <main className="min-h-screen bg-[#f7f9fc] px-5 py-5 font-['Inter'] text-slate-950">
      <section className="mx-auto w-full max-w-[430px]">
        ...
        <form onSubmit={save} className="mt-7 rounded-[28px] border border-cyan-200 bg-white p-5 shadow-[0_18px_55px_rgba(15,23,42,0.07)] ring-4 ring-cyan-50">
          {currentSlug.startsWith('kawvo-') && <div className="mb-5 rounded-2xl bg-amber-50 p-4 text-xs leading-5 text-amber-800">El usuario que ves ahora es temporal. Nadie espera que memorices ese código; cámbialo por uno tuyo.</div>}
          ...
          <button disabled={saving || normalizeSlug(slug).length < 2} className="mt-5 w-full rounded-2xl bg-slate-950 px-4 py-4 text-sm font-extrabold text-white disabled:opacity-35">{saving ? 'Reservando…' : 'Guardar mi usuario'}</button>
        </form>
      </section>
    </main>
  )
