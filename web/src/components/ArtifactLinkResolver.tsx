...
type ProductState = 'loading' | 'pending_activation' | 'profile_draft' | 'profile_draft_owner' | 'team_unassigned' | 'team_pending' | 'team_unavailable' | 'blocked' | 'unavailable' | 'not_ready' | 'later' | 'error'
...
export default function ArtifactLinkResolver() {
  ...
  const [independentConfirm, setIndependentConfirm] = useState(false)
  ...
  useEffect(() => {
    ...
    const inspect = async () => {
      ...
      try {
        ...
        setManageUrl(String(json.manage_url || ''))
  ...
  const inspectTeamCode = async () => {
    ...
    try {
      const response = await fetch(`${appOrigin()}/api/v1/public/team/code/inspect-v2`, {
  ...
  const continueTeam = () => {
    ...
    window.location.assign(`${appOrigin()}/admin/free/team/assign?public_code=${encodeURIComponent(code)}&team_code=${encodeURIComponent(teamCode)}`)
  }
      ...
      <section style={cardStyle}>
        ...
        {state === 'pending_activation' && (
          <>
            ...
            {!teamOpen && !independentConfirm && <div style={{ marginTop: 22 }}>
              <button type="button" onClick={() => setIndependentConfirm(true)} style={primaryButton}>Activación independiente</button>
              <button type="button" onClick={() => { setTeamOpen(true); setTeamError(''); setTeamResult(null) }} style={secondaryButton}>Vincular a un Team</button>
              <button type="button" onClick={() => setState('later')} style={secondaryButton}>Activar luego</button>
            </div>}

            {independentConfirm && <div style={{ marginTop: 22, textAlign: 'left' }}>
              <div style={{ border: '1px solid #fde68a', background: '#fffbeb', borderRadius: 18, padding: 16 }}>
                <p style={{ margin: 0, fontSize: 13, fontWeight: 900, color: '#92400e' }}>ACTIVACIÓN INDEPENDIENTE</p>
                <p style={{ margin: '8px 0 0', color: '#475569', lineHeight: 1.55, fontSize: 14 }}>Este dispositivo se activará de manera independiente y no quedará asociado a ningún Team.</p>
              </div>
              <button type="button" onClick={activateIndependent} disabled={starting} style={{ ...primaryButton, marginTop: 14, opacity: starting ? .6 : 1 }}>{starting ? 'Continuando…' : 'Sí, activar de forma independiente'}</button>
              <button type="button" onClick={() => setIndependentConfirm(false)} disabled={starting} style={secondaryButton}>Volver</button>
            </div>}

            {teamOpen && !teamResult && <div style={{ marginTop: 22, textAlign: 'left' }}>
              <h2 style={{ margin: 0, fontSize: 19 }}>Vincular a un Team</h2>
              <p style={{ margin: '8px 0 0', color: '#64748b', lineHeight: 1.55, fontSize: 14 }}>Ingresa el código de vinculación generado previamente por el Administrador Master del Team.</p>
            ...
            {teamOpen && teamResult && <div style={{ marginTop: 22, textAlign: 'left' }}>
              <div style={{ border: '1px solid #bae6fd', background: '#f0f9ff', borderRadius: 18, padding: 16 }}>
                ...
                <p style={{ margin: '8px 0 0', color: '#64748b', lineHeight: 1.5, fontSize: 13 }}>El Administrador Master preparará el perfil del colaborador y este código solo podrá utilizarse una vez.</p>
              </div>
              <button type="button" onClick={continueTeam} style={{ ...primaryButton, marginTop: 14 }}>Sí, preparar dispositivo para {teamResult.team_name}</button>
        ...
        {state === 'later' && <><div style={{ width: 48, height: 48, margin: '20px auto 0', display: 'grid', placeItems: 'center', borderRadius: '50%', background: '#f1f5f9', color: '#475569', fontSize: 22 }}>✓</div><h1 style={{ margin: '16px 0 8px', fontSize: 25 }}>Puedes activarlo cuando quieras</h1><p style={{ margin: 0, lineHeight: 1.6, color: '#64748b' }}>Tu producto sigue pendiente y disponible. Cuando estés listo, vuelve a escanear su QR o NFC.</p><button type="button" onClick={() => { setState('pending_activation'); setTeamOpen(false); setTeamResult(null); setIndependentConfirm(false) }} style={{ ...secondaryButton, marginTop: 20 }}>Volver</button></>}

        {(state === 'team_unassigned' || state === 'team_pending' || state === 'team_unavailable') && <>
          <div style={{ width: 48, height: 48, margin: '20px auto 0', display: 'grid', placeItems: 'center', borderRadius: '50%', background: state === 'team_unavailable' ? '#fff7ed' : '#f0f9ff', color: state === 'team_unavailable' ? '#c2410c' : '#0369a1', fontSize: 22, fontWeight: 900 }}>T</div>
          <p style={{ margin: '16px 0 0', fontSize: 12, fontWeight: 900, color: '#0369a1' }}>DISPOSITIVO TEAM</p>
          <h1 style={{ margin: '5px 0 8px', fontSize: 25 }}>{state === 'team_unassigned' ? 'Perfil no asignado' : state === 'team_pending' ? 'Perfil pendiente' : 'Perfil no disponible'}</h1>
          <p style={{ margin: 0, lineHeight: 1.6, color: '#64748b' }}>{message || 'Este dispositivo todavía no tiene un perfil público disponible.'}</p>
          {manageUrl && <button type="button" onClick={() => window.location.assign(manageUrl)} style={{ ...primaryButton, marginTop: 18 }}>Abrir en Team</button>}
        </>}
