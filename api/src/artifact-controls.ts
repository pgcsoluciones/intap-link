...
app.post('/api/v1/me/artifacts/:id/deactivate', requireArtifactOwner, async (c: any) => {
  ...
  return c.json({ ok: true, data: { status: 'suspended' } })
})
...
app.post('/api/v1/me/artifacts/:id/reactivate', requireArtifactOwner, async (c: any) => {
  ...
  await c.env.DB.prepare(
    `UPDATE intap_artifacts
      ...
      WHERE id = ? AND owner_user_id = ? AND status = 'suspended' AND profile_id = ?`,
  ).bind(artifactId, userId, profileId).run()

  return c.json({ ok: true, data: { status: 'activated' } })
})
...
app.post('/api/v1/me/artifacts/:id/unlink', requireArtifactOwner, async (c: any) => {
  ...
  await c.env.DB.prepare(
    `UPDATE intap_artifacts
        SET profile_id = NULL,
            status = CASE WHEN status = 'revoked' THEN status ELSE 'suspended' END,
...
app.post('/api/v1/me/artifacts/:id/link-current', requireArtifactOwner, async (c: any) => {
  ...
  await c.env.DB.prepare(
    `UPDATE intap_artifacts
        SET profile_id = ?, status = 'activated', updated_at = datetime('now')
  ...
  return c.json({
    ...
    data: {
      ...
      profile_slug: String((profile as any).slug || ''),
    },
  })
})
