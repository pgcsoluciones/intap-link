...
LOG_DIR="$ROOT/.preview-basic-cta-dedup-logs"
...
run python3 scripts/apply-basic-cta-dedup-2026-08-29.py
run git diff --check
...
if git diff --cached --quiet; then
  fail "No hubo cambios funcionales para confirmar."
fi

run git commit -m "fix(free): streamline basic plan CTAs and profile editing"
...
s2, n = re.subn(r'^APP_PAGES_ORIGIN = ".*"$', f'APP_PAGES_ORIGIN = "{origin}"', s, count=1, flags=re.M)
if n != 1:
...
if ! git diff --cached --quiet; then
  run git commit -m "chore(preview): pin basic CTA QA app deployment"
  run git push github "HEAD:$BRANCH"
fi
...
echo ""
echo "============================================================"
echo "✓ CTA / EDICIÓN PREVIEW COMPLETADO"
echo "============================================================"
echo "Branch:          $BRANCH"
