#!/usr/bin/env bash

# Kawvo Link · QA follow-up · SOLO PREVIEW
# Aplica, valida, confirma y despliega App + Worker Preview. No toca Producción.

ROOT="$HOME/Desktop/intap-link-universal-bilingual-audit"
BRANCH="fix/free-mobile-ai-bank-share"
APP_PROJECT="intap-link"
LOG_DIR="$ROOT/.preview-qa-followup-logs"

fail() {
  echo ""
  echo "✗ ERROR: $1"
  echo "Producción NO fue tocada. Revisa la salida anterior."
  exit 1
}

run() {
  echo ""
  echo "▶ $*"
  "$@" || fail "$*"
}

cd "$ROOT" || fail "No existe $ROOT"
mkdir -p "$LOG_DIR"

[ "$(git branch --show-current)" = "$BRANCH" ] || fail "Debes estar en $BRANCH"
[ -z "$(git status --porcelain)" ] || fail "El working tree no está limpio; no voy a mezclar cambios locales."

run git pull --ff-only github "$BRANCH"
run python3 scripts/apply-preview-qa-followup-2026-08-29.py
run git diff --check

echo ""
echo "▶ Validando App"
(cd app && npx tsc && npx vite build --mode preview) || fail "Build Preview de App"

echo ""
echo "▶ Validando API"
(cd api && npx tsc --noEmit) || fail "TypeScript de API"

# Verificación de esquema SOLO LECTURA en D1 Preview. La bandeja ya depende de esta tabla.
echo ""
echo "▶ Verificando tabla de notificaciones en D1 Preview"
NOTIFICATION_SCHEMA_LOG="$LOG_DIR/notification-schema-$(date +%Y%m%d-%H%M%S).log"
(cd api && npx wrangler d1 execute intap_db_preview --remote --config wrangler.preview.toml --command "SELECT name FROM sqlite_master WHERE type='table' AND name='user_notifications';") 2>&1 | tee "$NOTIFICATION_SCHEMA_LOG"
[ "${PIPESTATUS[0]}" -eq 0 ] || fail "Consulta de esquema D1 Preview"
grep -q "user_notifications" "$NOTIFICATION_SCHEMA_LOG" || fail "La tabla user_notifications no existe en Preview"

run git diff --check
run git add \
  api/src/index.ts \
  app/src/components/admin/free/FreeDashboard.tsx \
  app/src/components/admin/free/FreeVisualEditor.tsx \
  app/src/components/admin/free/FreeFirstRunGuide.tsx \
  app/src/components/admin/free/FreeBankAccounts.tsx \
  app/src/components/admin/free/FreeIdentifier.tsx \
  app/src/components/admin/free/onboarding/FreeArtifactActivation.tsx

if git diff --cached --quiet; then
  fail "El parche no produjo cambios para confirmar."
fi

run git commit -m "fix(free): refine preview UX and lifecycle notifications"
run git push github "HEAD:$BRANCH"

echo ""
echo "▶ Desplegando App SOLO a Pages Preview"
APP_LOG="$LOG_DIR/app-pages-$(date +%Y%m%d-%H%M%S).log"
(cd api && npx wrangler pages deploy ../app/dist --project-name "$APP_PROJECT" --branch "$BRANCH") 2>&1 | tee "$APP_LOG"
[ "${PIPESTATUS[0]}" -eq 0 ] || fail "Deploy Pages Preview de App"
APP_ORIGIN="$(grep -Eo 'https://[0-9a-f]{8,}\.intap-link\.pages\.dev' "$APP_LOG" | tail -1)"
[ -n "$APP_ORIGIN" ] || fail "No pude identificar el deployment inmutable de App"
echo "✓ APP_PAGES_ORIGIN=$APP_ORIGIN"

# Mantiene intacto WEB_PAGES_ORIGIN: este follow-up no modifica Web.
APP_ORIGIN="$APP_ORIGIN" python3 - <<'PY' || fail "Actualizar APP_PAGES_ORIGIN Preview"
from pathlib import Path
import os, re
p = Path('api/wrangler.preview.toml')
s = p.read_text()
app = os.environ['APP_ORIGIN']
s2, n = re.subn(r'^APP_PAGES_ORIGIN = ".*"$', f'APP_PAGES_ORIGIN = "{app}"', s, count=1, flags=re.M)
if n != 1:
    raise SystemExit('No pude actualizar APP_PAGES_ORIGIN')
p.write_text(s2)
print('✓ APP_PAGES_ORIGIN actualizado; WEB_PAGES_ORIGIN preservado')
PY

run git diff --check
run git add api/wrangler.preview.toml
if ! git diff --cached --quiet; then
  run git commit -m "chore(preview): pin QA follow-up app deployment"
  run git push github "HEAD:$BRANCH"
fi

echo ""
echo "▶ Desplegando Worker SOLO PREVIEW"
WORKER_LOG="$LOG_DIR/worker-$(date +%Y%m%d-%H%M%S).log"
(cd api && npx wrangler deploy --config wrangler.preview.toml) 2>&1 | tee "$WORKER_LOG"
[ "${PIPESTATUS[0]}" -eq 0 ] || fail "Deploy Worker Preview"
WORKER_VERSION="$(grep -E 'Current Version ID:' "$WORKER_LOG" | tail -1 | sed -E 's/.*Current Version ID:[[:space:]]*//')"
WEB_ORIGIN="$(grep -E '^WEB_PAGES_ORIGIN = ' api/wrangler.preview.toml | sed -E 's/^[^\"]*\"([^\"]+)\".*/\1/')"

echo ""
echo "============================================================"
echo "✓ QA FOLLOW-UP PREVIEW COMPLETADO"
echo "============================================================"
echo "Branch:          $BRANCH"
echo "Commit:          $(git rev-parse HEAD)"
echo "App Pages:       $APP_ORIGIN"
echo "Web Pages:       $WEB_ORIGIN (sin cambios)"
echo "Worker Version:  ${WORKER_VERSION:-ver salida de Wrangler}"
echo "App QA:          https://app.preview.intaprd.com/admin/free"
echo "Producción:      NO TOCADA"
echo "Logs:            $LOG_DIR"
echo "============================================================"
