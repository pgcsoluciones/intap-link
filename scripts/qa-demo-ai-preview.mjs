...
const crawlerHeaders = { 'user-agent': 'facebookexternalhit/1.1 (+http://www.facebook.com/externalhit_uatext.php)', accept: 'text/html' }
...
function session(label) { return `qa-${label}-${Date.now()}-${Math.random().toString(36).slice(2, 10)}` }
...

{
  ...
  assert.equal(result.response.status, 200); assert.equal(result.json?.data?.status, 'needs_more_info'); assert.ok(result.json.data.questions?.length >= 1)
...
}
{
  ...
  assert.equal(result.response.status, 400); console.log('✓ H: nombre vacío rechazado; IA no inventa identidad')
}
{
  ...
  assert.equal(result.response.status, 428); assert.equal(result.json?.code, 'consent_required'); console.log('✓ consentimiento: OpenAI bloqueado sin aceptación')
}
{
  ...
  assert.equal(second.response.status, 429); assert.equal(second.json?.code, 'cooldown'); assert.equal(second.json?.fallback, true)
...
}
{
  const snapshot = {
    ...
    }, layout: 'impacto', colors: { primary: '#111827', secondary: '#374151', accent: '#64748B', button: '#111827', background: '#F3F4F6', surface: '#FFFFFF', text: '#111827', heroGradient: '#111827' },
  }
  const form = new FormData(); form.set('snapshot', JSON.stringify(snapshot)); form.set('sector_key', 'Automotriz y mecánica'); form.set('session_key', session('share'))
  const created = await fetch(`${base}/api/v1/public/demo/share`, { method: 'POST', body: form })
  const createdJson = await created.json().catch(() => ({}))
  assert.equal(created.status, 201, `share create: ${JSON.stringify(createdJson)}`); assert.match(String(createdJson?.url || ''), /\/demo\/s\/[a-f0-9]{48}$/)
  const expiry = Date.parse(createdJson.expires_at); const hours = (expiry - Date.now()) / 3_600_000; assert.ok(hours > 23 && hours <= 24.1, `snapshot TTL debe ser ~24h, fue ${hours}`)
  const opened = await fetch(createdJson.url, { headers: crawlerHeaders }); assert.equal(opened.status, 200)
  const html = await opened.text(); const image = meta(html, 'og:image')
  assert.ok(image, 'demo compartida debe incluir og:image')
  assert.ok(!/favicon\.ico|\/assets\/og\/kawvo-link-og\.png/i.test(image), `demo compartida no debe usar imagen genérica: ${image}`)
  assert.match(image, /automotriz-mecanica-02\.webp/i, `demo debe compartir el retrato del perfil: ${image}`)
  const imageResponse = await fetch(image); assert.equal(imageResponse.status, 200, `og:image demo HTTP ${imageResponse.status}`); assert.match(imageResponse.headers.get('content-type') || '', /^image\//i)
  console.log(`✓ viralidad + Graph Card: demo compartida usa retrato real (${image})`)
}
