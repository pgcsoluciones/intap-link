...
export default function FreeTeamJoin() {
  ...
  useEffect(() => {
    ...
    apiPost('/public/team/code/inspect', { code, public_code: publicCode })
      .then((json: any) => {
        if (!json?.ok) setError(json?.error || 'El código Team ya no está disponible.')
        else setTeam(json.data)
      })
      ...
      <section className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-[430px] flex-col justify-center">
        <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_18px_55px_rgba(15,23,42,0.08)]">
          ...
          {loading ? <div className="py-8 text-center"><div className="mx-auto h-10 w-10 animate-spin rounded-full border-4 border-slate-200 border-t-cyan-500" /><p className="mt-4 text-sm font-bold text-slate-500">Validando vinculación…</p></div> : error ? <><h1 className="mt-3 text-2xl font-black">No pudimos continuar</h1><p className="mt-3 text-sm leading-6 text-rose-600">{error}</p><button type="button" onClick={() => navigate('/admin/free')} className="mt-5 w-full rounded-2xl bg-slate-950 px-4 py-4 text-sm font-black text-white">Ir a mi panel</button></> : <>
            <h1 className="mt-3 text-[28px] font-black leading-tight">Vincular al Team</h1>
            <p className="mt-3 text-sm leading-6 text-slate-500">Tu producto quedará conectado a <strong>{team?.master_name}</strong>. El código se utilizará una sola vez.</p>
            ...
            <button type="button" onClick={() => void join()} disabled={joining} className="mt-5 w-full rounded-2xl bg-slate-950 px-4 py-4 text-sm font-black text-white disabled:opacity-40">{joining ? 'Vinculando…' : 'Confirmar vinculación'}</button>
            <button type="button" onClick={() => navigate('/admin/free')} disabled={joining} className="mt-3 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3.5 text-sm font-black text-slate-600">Vincular luego</button>
          </>}
        </div>
      </section>
    </main>
