-- KawLink Sponsored Profile V1
-- Isolated sponsored-product model. Does not alter Free/Plus/Team profile tables.
...
CREATE TABLE IF NOT EXISTS sponsor_batches (
  id TEXT PRIMARY KEY,
  sponsor_id TEXT NOT NULL,
  name TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS sponsored_profiles (
  ...
  cover_variant TEXT NOT NULL DEFAULT 'standard',
  phone TEXT,
  whatsapp TEXT,
  instagram TEXT,
  address TEXT,
  ...
  gallery_title TEXT NOT NULL DEFAULT 'Catálogo',
  palette_id TEXT NOT NULL DEFAULT 'blue',
  status TEXT NOT NULL DEFAULT 'draft' CHECK (status IN ('draft','published','suspended')),
  modules_json TEXT NOT NULL DEFAULT '[]',
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at TEXT NOT NULL DEFAULT (datetime('now')),
  published_at TEXT,
  FOREIGN KEY (sponsor_id) REFERENCES sponsor_tenants(id) ON DELETE RESTRICT
);
...
CREATE TABLE IF NOT EXISTS sponsor_artifacts (
  ...
  batch_id TEXT,
  sponsored_profile_id TEXT,
  beneficiary_user_id TEXT,
  status TEXT NOT NULL DEFAULT 'available' CHECK (status IN ('available','activated','inactive')),
  activated_at TEXT,
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  PRIMARY KEY (sponsor_id, artifact_id),
  FOREIGN KEY (sponsor_id) REFERENCES sponsor_tenants(id) ON DELETE CASCADE,
  FOREIGN KEY (batch_id) REFERENCES sponsor_batches(id) ON DELETE SET NULL,
  FOREIGN KEY (sponsored_profile_id) REFERENCES sponsored_profiles(id) ON DELETE SET NULL
);

CREATE TABLE IF NOT EXISTS sponsor_module_grants (
  sponsor_id TEXT NOT NULL,
  module_code TEXT NOT NULL,
  enabled INTEGER NOT NULL DEFAULT 1 CHECK (enabled IN (0,1)),
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at TEXT NOT NULL DEFAULT (datetime('now')),
  PRIMARY KEY (sponsor_id, module_code),
  FOREIGN KEY (sponsor_id) REFERENCES sponsor_tenants(id) ON DELETE CASCADE
);
