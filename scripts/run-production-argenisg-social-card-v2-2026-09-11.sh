#!/usr/bin/env bash
set -euo pipefail

ROOT="$HOME/Desktop/intap-link-universal-bilingual-audit"
VENV="$ROOT/.venv-adonisg-assets"
OG_LOCAL="$ROOT/web/public/assets/adonisg/og/adonisg-og-v2.jpg"
BUCKET="intap-r2"
R2_KEY="profile-assets/argenisg/v1/og/adonisg-og-v2.jpg"
PROFILE_ID="profile-managed-argenisg"
WEB_PROJECT="intap-link"
TMP_SQL="/tmp/argenisg-og-v2-$$.sql"

fail(){ echo "✗ ERROR: $*" >&2; exit 1; }
run(){ echo "▶ $*"; "$@"; }
cleanup(){ rm -f "$TMP_SQL"; rm -rf "$ROOT/web/public/assets/adonisg/og"; }
trap cleanup EXIT

cd "$ROOT"

echo '============================================================'
echo 'KAWVO LINK · /argenisg · SOCIAL CARD V2'
echo '============================================================'
echo '- 1200x630'
echo '- sujeto contenido, sin crop vertical'
echo '- nueva URL para evitar caché social anterior'
echo '============================================================'

run git checkout main
run git pull --ff-only github main
[ -z "$(git status --porcelain)" ] || fail 'El árbol de trabajo no está limpio.'

if [ ! -x "$VENV/bin/python" ]; then run python3 -m venv "$VENV"; fi
run "$VENV/bin/python" -m pip install --quiet --upgrade pip Pillow
run "$VENV/bin/python" scripts/prepare-argenisg-og-v2.py

run "$VENV/bin/python" - "$OG_LOCAL" <<'PY'
from PIL import Image
import sys
p=sys.argv[1]
with Image.open(p) as im:
    assert im.size == (1200,630), im.size
    assert im.format == 'JPEG', im.format
print('✓ OG validada 1200x630 JPEG')
PY

echo '▶ Subir OG V2 a R2'
(
  cd api
  npx wrangler r2 object put "$BUCKET/$R2_KEY" --file "$OG_LOCAL" --content-type image/jpeg >/dev/null
)

cat > "$TMP_SQL" <<SQL
INSERT INTO profile_assets (profile_id,asset_key,r2_key,kind,content_type,sort_order,is_active,metadata_json,updated_at)
VALUES ('$PROFILE_ID','og/adonisg-og-v2.jpg','$R2_KEY','image','image/jpeg',999,1,'{}',datetime('now'))
ON CONFLICT(profile_id,asset_key) DO UPDATE SET r2_key=excluded.r2_key,content_type=excluded.content_type,is_active=1,updated_at=datetime('now');
UPDATE profiles
SET template_data=json_set(CASE WHEN json_valid(template_data) THEN template_data ELSE '{}' END,
  '$.social_image_url','https://intaprd.com/assets/adonisg/og/adonisg-og-v2.jpg'),
  updated_at=datetime('now')
WHERE id='$PROFILE_ID';
SQL
(
  cd api
  npx wrangler d1 execute intap_db --remote --config wrangler.toml --file "$TMP_SQL"
)

run python3 - <<'PY'
from pathlib import Path
files = [
    Path('functions/_middleware.ts'),
    Path('functions/profile-discovery.ts'),
    Path('web/src/components/profile-templates/IntapProfileAdonisgV1.tsx'),
    Path('api/migrations/0046_argenisg_r2_assets.sql'),
    Path('api/migrations-preview/0047_argenisg_r2_assets.sql'),
]
old='adonisg-og.jpg'
new='adonisg-og-v2.jpg'
for p in files:
    text=p.read_text(encoding='utf-8')
    if new in text:
        print(f'✓ {p}: ya usa V2')
        continue
    if old not in text:
        raise SystemExit(f'No encontré {old} en {p}')
    p.write_text(text.replace(old,new),encoding='utf-8')
    print(f'✓ {p}: OG -> V2')
PY

run git diff --check
run npm run build -w web

run git add functions/_middleware.ts functions/profile-discovery.ts web/src/components/profile-templates/IntapProfileAdonisgV1.tsx api/migrations/0046_argenisg_r2_assets.sql api/migrations-preview/0047_argenisg_r2_assets.sql
run git commit -m 'fix: use uncropped argenisg social card v2'
run git push github main

# generated local OG must not enter Pages static bundle; R2 is the source of truth
rm -rf "$ROOT/web/public/assets/adonisg/og"
run npm run build -w web
run npx wrangler pages deploy web/dist --project-name "$WEB_PROJECT" --branch main

BASE='https://intaprd.com'
code="$(curl -sS -L -o /dev/null -w '%{http_code}' "$BASE/assets/adonisg/og/adonisg-og-v2.jpg")"
[ "$code" = 200 ] || fail "OG V2 respondió HTTP $code"

html="$(curl -sS -A 'Twitterbot/1.0' "$BASE/argenisg")"
printf '%s' "$html" | grep -Fq 'https://intaprd.com/assets/adonisg/og/adonisg-og-v2.jpg' || fail 'Crawler aún no recibe OG V2'
printf '%s' "$html" | grep -Fq 'summary_large_image' || fail 'Falta summary_large_image'

echo '============================================================'
echo '✓ SOCIAL CARD /argenisg V2 PUBLICADA'
echo '✓ 1200x630 sin cortar el sujeto'
echo '✓ R2 + D1 + metadata server-side actualizados'
echo '============================================================'
