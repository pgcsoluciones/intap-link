...
export default function FreeDashboard() {
  ...
  return <>
    ...
    <main className="min-h-screen bg-[#f7f9fc] pb-24 font-['Inter'] text-slate-950">
      <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/95 px-5 py-4 backdrop-blur"><div className="mx-auto flex w-full max-w-[430px] items-center justify-between gap-3"><div><p className="text-[11px] font-black uppercase tracking-[0.2em] text-cyan-600">KAWVO LINK</p><h1 className="mt-0.5 text-xl font-black tracking-[-0.03em]">Mi panel</h1></div><div className="flex items-center gap-2"><FreeNotificationBell />{hasSuperAdminAccess && <button type="button" onClick={() => navigate('/superadmin')} className="rounded-full bg-slate-950 px-3 py-2 text-xs font-black text-white">Super Admin</button>}<button type="button" onClick={() => navigate('/admin/free/account')} className="rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 shadow-sm">Mi cuenta</button></div></div></header>

      <section className="mx-auto w-full max-w-[430px] space-y-4 px-5 pt-5">
        {isTeamMember && <button type="button" onClick={() => navigate('/admin/free/team/member')} className="flex w-full items-center justify-between rounded-[22px] border border-amber-300 bg-amber-50 p-4 text-left"><span><span className="block text-[10px] font-black uppercase tracking-[0.14em] text-amber-800">Perfil vinculado a Team</span><span className="mt-1 block text-sm font-black">{teamContext.member?.master_name || 'Perfil master'}</span><span className="mt-1 block text-xs text-amber-800">Revisa qué opciones puedes modificar.</span></span><span className="text-xl">›</span></button>}
        {teamContext.role === 'master' && <button type="button" onClick={() => navigate('/admin/free/team')} className="flex w-full items-center justify-between rounded-[22px] border border-cyan-200 bg-white p-4 text-left shadow-sm"><span><span className="block text-[10px] font-black uppercase tracking-[0.14em] text-cyan-700">Kawvo Link Team</span><span className="mt-1 block text-sm font-black">Administrar equipo y códigos</span><span className="mt-1 block text-xs text-slate-500">Genera códigos, permisos y revisa dispositivos vinculados.</span></span><span className="text-xl text-cyan-700">›</span></button>}
        {teamContext.role === 'none' && <button type="button" onClick={() => navigate('/admin/free/team')} className="flex w-full items-center justify-between rounded-[22px] border border-slate-200 bg-white p-4 text-left shadow-sm"><span><span className="block text-[10px] font-black uppercase tracking-[0.14em] text-slate-400">Team</span><span className="mt-1 block text-sm font-black">Crear equipo de trabajo</span><span className="mt-1 block text-xs text-slate-500">Vincula dispositivos a este perfil master.</span></span><span className="text-xl text-slate-400">›</span></button>}
