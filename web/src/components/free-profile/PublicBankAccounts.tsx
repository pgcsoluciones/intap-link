...
export default function PublicBankAccounts() {
  ...
  async function copyText(value: string, key: string) {
    ...
    setCopiedKey(key); window.setTimeout(() => setCopiedKey((current) => current === key ? null : current), 1800)
  }
  ...
  async function copyHolderId(account: PublicBankAccount) {
    ...
    try {
      ...
      const json = await response.json(); if (!json?.ok || !json.data?.copy_value) return; await copyText(String(json.data.copy_value), `id:${account.id}`)
  ...
  async function copyBankSectionLink() { await copyText(bankSectionUrl(), 'bank-link') }
  if (!enabled || items.length === 0 || !portalHost) return null

  const content = (
    <section id="bancos" className="ilx-section scroll-mt-5" aria-labelledby="ilx-bank-title" style={{ borderTop: '1px solid var(--ilx-border)', paddingTop: 22, marginTop: 22 }}>
      <div>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <button type="button" onClick={shareBankSectionWhatsApp} className="rounded-xl border px-3 py-2.5 text-xs font-black transition active:scale-[0.98]" style={{ borderColor: 'var(--ilx-border)', background: 'var(--ilx-soft-primary)', color: 'var(--ilx-primary)' }} aria-label="Compartir enlace de cuentas bancarias por WhatsApp">Enviar por WhatsApp</button>
          <button type="button" onClick={() => void copyBankSectionLink()} className="rounded-xl border px-3 py-2.5 text-xs font-black transition active:scale-[0.98]" style={{ borderColor: 'var(--ilx-border)', background: 'var(--ilx-surface)', color: 'var(--ilx-text)' }} aria-label="Copiar enlace directo a cuentas bancarias" aria-live="polite">{copiedKey === 'bank-link' ? '✓ Enlace copiado' : 'Copiar enlace'}</button>
        </div>
      </div>
            ...
            <article key={account.id} className="rounded-[20px] border p-4" style={{ borderColor: 'var(--ilx-border)', background: 'var(--ilx-soft-primary)' }}>
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1 pt-1">
                  ...
                  {account.holder_id_type && <p className="mt-1.5 text-[11px] font-semibold" style={{ color: 'var(--ilx-muted)' }}>{account.holder_id_type === 'rnc' ? 'RNC' : 'Cédula'} protegido · se copia sin mostrarse</p>}
                </div>
              </div>
              <div className="mt-4 grid grid-cols-2 gap-2">
                <button type="button" onClick={() => void copyText(account.copy_value, `account:${account.id}`)} className="rounded-xl px-3 py-3 text-sm font-black transition active:scale-[0.98]" style={copiedKey === `account:${account.id}` ? { background: '#D1FAE5', color: '#065F46' } : { background: 'var(--ilx-action)', color: 'var(--ilx-on-action)' }} aria-live="polite">{copiedKey === `account:${account.id}` ? '✓ Cuenta copiada' : 'Copiar cuenta'}</button>
                <button type="button" disabled={!account.holder_id_type} onClick={() => void copyHolderId(account)} className="rounded-xl border px-3 py-3 text-sm font-black transition active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-45" style={copiedKey === `id:${account.id}` ? { background: '#D1FAE5', color: '#065F46', borderColor: '#A7F3D0' } : { background: 'var(--ilx-surface)', color: 'var(--ilx-text)', borderColor: 'var(--ilx-border)' }} aria-live="polite">{copiedKey === `id:${account.id}` ? `✓ ${account.holder_id_type === 'rnc' ? 'RNC' : 'Cédula'} copiado` : `Copiar ${account.holder_id_type === 'rnc' ? 'RNC' : 'cédula'}`}</button>
              </div>
            </article>
          )
        })}
      </div>
      <p className="mt-3 text-center text-[11px] font-semibold" style={{ color: 'var(--ilx-muted)' }}>Tus datos de identificación no se muestran públicamente.</p>
    </section>
  )
