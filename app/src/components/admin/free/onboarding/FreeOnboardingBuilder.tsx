...
const STATUS_MESSAGES = [
  'Nuestro equipo está preparando tu perfil base.',
  'Estamos adaptando la base a tu actividad comercial.',
  ...
  'Ya casi… estamos dando los últimos toques.',
  'Tu borrador estará listo en un momento.',
]

const BUILD_MS = 15000
...
export default function FreeOnboardingBuilder() {
  ...
  useEffect(() => {
    ...
    const build = async () => {
      ...
      if (!profileResult.ok) {
        if (!cancelled) setError(profileResult.error || 'No pudimos preparar tu perfil base.')
        return
      }

      const starterResult: any = await apiPost('/me/free/starter/apply', {
        category,
        subcategory,
        variant: 1,
      }).catch(() => ({ ok: false, error: 'No pudimos colocar el contenido base en tu borrador.' }))

      if (!starterResult.ok) {
        if (!cancelled) setError(starterResult.error || 'No pudimos completar tu perfil base.')
        return
      }
      ...
      const minimumWait = Math.max(0, BUILD_MS - (Date.now() - startedAt))
      window.setTimeout(() => {
        if (!cancelled) {
          setProgress(100)
          setDone(true)
      ...
      <main className="relative min-h-screen overflow-hidden bg-[#f7f9fc] px-4 py-7 font-['Inter'] text-slate-950 sm:px-5 sm:py-8">
        ...
        <section className="relative z-10 mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-[430px] flex-col items-center justify-center text-center">
          <div className="w-full rounded-[30px] border border-slate-200 bg-white p-6 shadow-[0_18px_55px_rgba(15,23,42,0.08)]">
            ...
            <p className="mt-4 text-base font-medium leading-7 text-slate-700">Ya lo convertimos en tu borrador para que puedas verlo y empezar a personalizarlo de inmediato.</p>
            <div className="mt-5 rounded-2xl bg-cyan-100 p-4 text-base font-semibold leading-7 text-slate-800">Contiene textos e imágenes de ejemplo. Antes de publicar debes reemplazarlos por tus datos, fotos y contenido reales.</div>
            <button type="button" onClick={() => navigate('/admin/free/onboarding/review')} className="mt-6 w-full rounded-2xl bg-slate-950 px-4 py-4 text-base font-extrabold text-white">Ver mi borrador</button>
          </div>
        </section>
      </main>
    )
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f7f9fc] px-4 py-7 font-['Inter'] text-slate-950 sm:px-5 sm:py-8">
      ...
      <section className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-[430px] flex-col items-center justify-center text-center">
        ...
        <h1 className="mt-3 text-[28px] font-black leading-tight tracking-[-0.04em]">Estamos preparando tu perfil base</h1>
