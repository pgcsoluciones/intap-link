...
export default function FreeBankAccounts() {
  ...
  async function saveAccount() {
    ...
    if (!form.bank_name.trim() || !form.account_number.trim() || !form.holder_name.trim()) {
      setMessage('Completa banco, número de cuenta y titular.')
      return
    }
    setSaving(true)
    setMessage('')
      ...
      <main className="min-h-screen bg-[#f7f9fc] px-4 py-5 font-['Inter'] text-slate-950">
        <section className="mx-auto w-full max-w-[620px]">
          ...
          <div className="mt-5 rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-3 text-base font-medium leading-7 text-slate-600">Agrega hasta 3 cuentas a tu perfil y permite que tus clientes copien el número con un toque.</p>
  ...
  return (
    <main className="min-h-screen bg-[#f7f9fc] pb-24 font-['Inter'] text-slate-950">
      <section className="mx-auto w-full max-w-[760px] px-4 py-5 sm:px-5">
        ...
        <div className="mt-5">
          ...
          <div className="mt-2 flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-xl text-base font-medium leading-7 text-slate-600">Muestra hasta 3 cuentas para que tus clientes puedan copiar el número fácilmente.</p>
            </div>
            {access.source === 'fair' && <span className="rounded-full bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700">Beneficio de feria · Permanente</span>}
          </div>
        </div>
        ...
        {message && <p className="mt-4 rounded-2xl bg-white px-4 py-3 text-center text-sm font-black text-slate-700 shadow-sm">{message}</p>}
