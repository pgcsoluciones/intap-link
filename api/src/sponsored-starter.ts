...
app.post('/api/v1/me/sponsored-profile/starter',async(c:any)=>{
  ...
  const profile=await c.env.DB.prepare(`SELECT id,profile_role,business_name,username FROM sponsored_profiles WHERE user_id=? ORDER BY created_at DESC LIMIT 1`).bind(userId).first()
  ...
  const businessName=existingName||(role==='sponsor_owner'?'Tu empresa':'Tu nombre o negocio')
