...
LOG_DIR="$ROOT/.preview-image-bank-og-logs"
...
run python3 scripts/apply-image-upload-and-bank-og-fix-2026-08-29.py
run git diff --check
...
if git diff --cached --quiet; then
  fail "No hay cambios funcionales para confirmar"
fi

run git commit -m "fix(free): harden image upload feedback and bank social card"
...
[ -n "$WEB_ORIGIN" ] || fail "No pude identificar Web Pages origin"
...
if ! git diff --cached --quiet; then
  run git commit -m "chore(preview): pin image and bank OG QA deployments"
  run git push github "HEAD:$BRANCH"
fi
...
echo ""
echo "============================================================"
echo "✓ IMÁGENES / BANK OG PREVIEW COMPLETADO"
echo "============================================================"
echo "Branch:          $BRANCH"
