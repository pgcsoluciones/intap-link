...
export default function KawvoTrial({mode}:{mode:Mode}){
  ...
  return <>
    ...
    {published&&<div className="trial-sheet-backdrop"><section className="trial-sheet trial-success"><button className="trial-success-close" type="button" onClick={()=>navigate('/trial')} aria-label="Cerrar"><FaTimes/></button><FaCheck className="trial-success-icon"/><h2>Trial creado</h2><p>Activo hasta {isoDisplay(published.expires_at)}.</p>{qr&&<img src={qr} alt="Código QR del Trial"/>}<strong>{window.location.origin}{published.url}</strong><div className="trial-success-actions"><button onClick={()=>copy(window.location.origin+published.url)}><FaCopy/> Copiar enlace</button><button onClick={()=>share(window.location.origin+published.url)}><FaShareAlt/> Compartir</button><button onClick={()=>window.open(published.url,'_blank')}><FaQrcode/> Abrir perfil</button></div><button className="trial-publish" onClick={()=>navigate(published.url)}>Ver Trial publicado</button><button className="trial-success-dismiss" type="button" onClick={()=>navigate('/trial')}>Cerrar y volver a Trials</button></section></div>}
