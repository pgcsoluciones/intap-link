...
export default function KawvoTrial({mode}:{mode:Mode}){
  ...
  function addBank(){setSnapshot(current=>{const bank=current.modules?.banks||{enabled:true,items:[]};if(bank.items.length>=3)return current;const item:TrialBank={id:crypto.randomUUID(),bank_name:'Banco Popular Dominicano',account_number:'',account_type:'savings',currency:'DOP',holder_name:current.profile.name,holder_id_type:'cedula',holder_id_number:'',display_mode:'masked'};const next={...current,modules:{...current.modules,banks:{enabled:true,items:[...bank.items,item]}}};queueSave(next);return next})}
