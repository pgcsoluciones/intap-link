...
LOG_DIR="/tmp/kawvo-team-contact-vcard-crop-2026-09-09"
...
echo "============================================================"
echo "KAWVO TEAM · CONTACTOS + VCARD + CROP · PRODUCCION"
echo "============================================================"
echo "Incluye:"
...
def replace_once(path, old, new):
    ...
    s=p.read_text()
    if old not in s:
        raise SystemExit(f"Patrón no encontrado en {path}: {old[:90]!r}")
    if s.count(old)!=1:
        raise SystemExit(f"Patrón ambiguo en {path}: {s.count(old)} coincidencias")
    p.write_text(s.replace(old,new,1))

# 1) Editor Team: usar el mismo ImageCropModal del perfil independiente.
...
# 2) Modelo público: guardar teléfono, WhatsApp y email por separado.
path='web/src/components/free-profile/IntapLinkGratis.types.ts'
replace_once(path,
"  phone: string\n  whatsappGreetingName: string",
"  phone: string\n  whatsapp: string\n  email: string\n  whatsappGreetingName: string")

# 3) Adapter: no usar WhatsApp como fuente del teléfono y hacer email/WhatsApp member-aware.
...
# 4) Perfil público: CTA WhatsApp usa WhatsApp; vCard guarda los tres datos independientes.
...
TAG="prod-team-contact-vcard-crop-2026-09-09-$(date +%H%M%S)"
git tag -a "$TAG" "$PRODUCT_SHA" -m "Team contact vCard and avatar crop release"
git push github "$TAG"

echo
echo "============================================================"
echo "✓ KAWVO TEAM · CONTACTOS + VCARD + CROP DESPLEGADO"
echo "============================================================"
...
echo "Servicios: pendiente de la referencia visual que enviará el usuario"
echo "Logs: $LOG_DIR"
echo "============================================================"
