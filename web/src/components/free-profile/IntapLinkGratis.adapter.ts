...
const SERVICE_ICON_SEQUENCE: FreeProfileServiceIconKey[] = [
  'home',
  'key',
  'chart-line',
  'handshake',
...
const QUICK_ACTION_TYPES = new Set<FreeProfileQuickActionType>([
  'call',
  'instagram',
  'location',
  'email',
...
function findLinkUrl(
  links: UnknownRecord[],
  matcher: (label: string, url: string) => boolean,
): string {
...
function findSocialUrl(socialLinks: UnknownRecord[], type: string): string {
  ...
  const match = socialLinks.find(
    (link) => normalizeText(readString(link, 'type')) === normalizedType,
  )
  return match ? readString(match, 'url') : ''
}
...
function resolvePortfolio(data: UnknownRecord): FreeProfileData['portfolio'] {
  return readRecords(data, 'gallery')
    .map((item, index) => ({
...
function resolveServices(data: UnknownRecord): FreeProfileService[] {
  return readRecords(data, 'products')
    .map((item, index) => {
      ...
      const iconKey = isServiceIconKey(iconToken)
        ? iconToken
        : SERVICE_ICON_SEQUENCE[index % SERVICE_ICON_SEQUENCE.length]

      return {
        id: readString(item, 'id') || `service-${index + 1}`,
        title,
        description: readString(item, 'description') || 'Solicita más información sobre este servicio.',
...
function resolveQuickActions(
  ...
  const selected = socialLinks
    .map((link) => {
      ...
      return {
        type,
        label: QUICK_ACTION_LABELS[type],
        url,
        sortOrder: Number(link.sort_order ?? link.sortOrder ?? 999),
      }
    })
    .filter((item): item is FreeProfileQuickAction & { sortOrder: number } => Boolean(item))
...
function normalizeHexColor(value: string): string {
  ...
  if (/^#[0-9a-f]{3}$/i.test(trimmed)) {
    return '#' + trimmed.slice(1).split('').map((character) => character.repeat(2)).join('')
  }
  return ''
}
...
function buildAvatarPlaceholder(name: string, colors: FreeProfileAppearanceColors): string {
  ...
  const svg =
    ...
    `<text x="300" y="335" text-anchor="middle" font-family="Arial, sans-serif" font-size="170" font-weight="700" fill="#ffffff">${initials}</text>` +
    `</svg>`
...
export function resolveFreeProfileAppearanceColors(payload: unknown): FreeProfileAppearanceColors {
  ...
  const brandColor = pickColor(
    ...
    readString(data, 'accentColor', 'accent_color'),
  )
...
function isFreeProfileLayoutId(value: string): value is FreeProfileLayoutId {
...
export function resolveFreeProfileLayout(payload: unknown): FreeProfileLayoutId {
  ...
  const templateData = readObject(data, 'templateData')
  const candidate =
    readString(data, 'layoutId', 'layout_id') ||
    readString(templateData, 'layout_id')
...
export function adaptPublicProfileApiResponse(payload: unknown): FreeProfileAdapterResult {
  ...
  const phoneSource =
    readString(data, 'whatsapp_number', 'whatsappNumber') ||
    readString(contact, 'whatsapp', 'phone') ||
    whatsappLink
  const phone = normalizePhone(phoneSource)
  const instagram =
    findSocialUrl(socialLinks, 'instagram') ||
    findSocialUrl(socialLinks, 'free_instagram') ||
    findLinkUrl(links, (_, url) => url.includes('instagram.com'))
  const location =
    findSocialUrl(socialLinks, 'free_location') ||
    readString(contact, 'map_url') ||
    mapLink

  const category = readString(data, 'category')
  const role =
    readString(templateData, 'role', 'title') ||
    readString(data, 'subcategory', 'category') ||
    starter.role
  const greetingName =
    readString(templateData, 'whatsapp_greeting_name') ||
    name.split(/\s+/)[0] ||
    name
  const portrait =
    readString(data, 'avatarUrl', 'avatar_url') ||
    buildAvatarPlaceholder(name, colors)
  const hero =
    readString(data, 'heroUrl', 'hero_url') ||
    readString(templateData, 'hero_url')
  ...
  return {
    ...
    profile: {
      ...
      aboutTitle: readString(templateData, 'about_title', 'headline') || `Conoce más sobre ${name}.`,
