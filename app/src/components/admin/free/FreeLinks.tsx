...
export default function FreeLinks() {
  ...
  const saveEdit = async (item: LinkItem) => {
    ...
    setError('')
    try {
      const json: any = await apiPut(`/me/links/${item.id}`, {
  ...
  return (
    <main className="min-h-screen bg-[#f7f9fc] font-['Inter'] text-slate-950">
      <div className="mx-auto w-full max-w-[430px] px-5 pb-24 pt-5">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-500">Agrega hasta 3 accesos importantes para tu perfil. Aunque alcances el límite, siempre podrás editar, ocultar o eliminar los existentes.</p>
              ...
              {editingId === item.id ? (
                <div>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    <button
                      type="button"
                      onClick={cancelEdit}
                      className="rounded-xl bg-slate-50 px-3 py-2.5 text-xs font-black text-slate-600"
                    >
                      Cancelar
                    </button>
                    <button
                      type="button"
                      disabled={saving || !editLabel.trim() || !editUrl.trim()}
                      onClick={() => void saveEdit(item)}
                      className="rounded-xl bg-cyan-600 px-3 py-2.5 text-xs font-black text-white disabled:opacity-40"
                    >
                ...
                <>
                  ...
                  <div className="mt-4 grid grid-cols-3 gap-2 border-t border-slate-100 pt-3">
                    <button onClick={() => startEdit(item)} className="rounded-xl bg-cyan-50 px-3 py-2 text-xs font-bold text-cyan-700">Editar</button>
                    <button onClick={() => void toggle(item)} className="rounded-xl bg-slate-50 px-3 py-2 text-xs font-bold text-slate-600">{item.is_active === 0 ? 'Mostrar' : 'Ocultar'}</button>
