...
export default function SuperAdminSupport() {
  ...
  async function respond() {
    if (!selected || saving || note.trim().length < 2) return
    setSaving(true)
    setError('')
    setFeedback('')
    try {
      ...
      if (replyChannel === 'whatsapp' && json.data?.whatsapp_url) {
        ...
        setFeedback('Respuesta registrada. Se abrió WhatsApp con el mensaje preparado para enviarlo al usuario.')
      } else if (replyChannel === 'email') {
        setFeedback('Respuesta enviada por correo y guardada en el historial del ticket.')
      } else {
        setFeedback('Respuesta publicada en el panel del usuario.')
      }
      await load()
      const detail: any = await apiGet(`/superadmin/support-tickets/${selected.id}`)
      if (detail?.ok) setSelectedDetail(detail.data)
                ...
                {selectedId === ticket.id && selected && (
                  <div style={{ marginTop: 16, borderTop: '1px solid #e2e8f0', paddingTop: 16, display: 'grid', gap: 16 }}>
                      ...
                      <div style={{ display: 'grid', gap: 8 }}>
                        {(selected.events || []).map((event) => (
                          <div key={event.id} style={{ display: 'grid', gridTemplateColumns: '12px 1fr', gap: 10, alignItems: 'start' }}>
                            <span style={{ width: 10, height: 10, borderRadius: 999, background: event.status_key === 'responded' ? '#10b981' : event.status_key === 'in_progress' ? '#0ea5e9' : '#94a3b8', marginTop: 5 }} />
                            <div><strong style={{ fontSize: 12 }}>{event.status_key === 'submitted' ? 'Enviada' : event.status_key === 'received' ? 'Recibida' : event.status_key === 'in_progress' ? 'En proceso' : event.status_key === 'responded' ? 'Respuesta' : 'Cerrado'}</strong>{event.channel && <span style={{ marginLeft: 6, fontSize: 11, color: '#64748b' }}>· {channelLabel(event.channel)}</span>}<div style={{ marginTop: 2, fontSize: 12, color: '#64748b' }}>{event.message || ''}</div><div style={{ marginTop: 2, fontSize: 10, color: '#94a3b8' }}>{event.created_at || ''}</div></div>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <label style={{ display: 'block', fontSize: 12, fontWeight: 900, color: '#334155' }}>
                      ...
                      <textarea value={note} onChange={(event) => setNote(event.target.value)} rows={5} placeholder="Escribe aquí la respuesta para el usuario…" style={{ marginTop: 8, width: '100%', boxSizing: 'border-box', border: '1px solid #cbd5e1', borderRadius: 12, padding: 12, resize: 'vertical', font: 'inherit' }} />
                    </label>

                    <div style={{ display: 'grid', gap: 8 }}>
                      <label style={{ fontSize: 12, fontWeight: 900, color: '#334155' }}>Canal de respuesta</label>
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {(['system', 'email', 'whatsapp'] as const).map((channel) => (
                          <button key={channel} type="button" onClick={() => setReplyChannel(channel)} style={{ border: replyChannel === channel ? '2px solid #0891b2' : '1px solid #cbd5e1', borderRadius: 12, padding: '9px 12px', background: replyChannel === channel ? '#ecfeff' : '#fff', color: '#334155', fontWeight: 900, cursor: 'pointer' }}>{channelLabel(channel)}</button>
                        ))}
                      </div>
                      <div style={{ fontSize: 11, lineHeight: 1.5, color: '#64748b' }}>{replyChannel === 'system' ? 'El usuario verá la respuesta dentro de su historial de Soporte Kawvo.' : replyChannel === 'email' ? `Se enviará al correo ${selected.user_email || selected.profile_email || 'registrado'} y también quedará guardada en el ticket.` : `Se abrirá WhatsApp con el mensaje preparado para ${selected.user_phone || 'el número registrado'}. La respuesta también quedará registrada en el ticket.`}</div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      ...
                      <button type="button" disabled={saving || note.trim().length < 2} onClick={() => void respond()} style={{ border: 0, borderRadius: 11, padding: '10px 14px', background: '#0f172a', color: '#fff', fontWeight: 900, cursor: 'pointer' }}>{saving ? 'Procesando…' : replyChannel === 'whatsapp' ? 'Responder por WhatsApp' : replyChannel === 'email' ? 'Enviar por correo' : 'Responder en el sistema'}</button>
