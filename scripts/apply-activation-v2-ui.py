...
# 1) Replace activation UI with public-code -> account/profile -> secret -> atomic claim.
activation = Path('app/src/components/admin/ArtifactActivation.tsx')
activation.write_text(r'''import { useEffect, useState } from 'react'
...
export function ArtifactManager() {
  ...
  return <main className="min-h-screen bg-[#f7f9fc] px-5 py-8 font-['Inter'] text-slate-950"><section className="mx-auto w-full max-w-3xl"><Link to="/admin" className="text-xs font-bold text-slate-500">← Volver al panel</Link><div className="mt-5 mb-7 flex items-end justify-between gap-4"><div><p className="text-[11px] font-black uppercase tracking-[0.22em] text-cyan-600">INTAP LINK</p><h1 className="mt-2 text-3xl font-black">Mis productos físicos</h1></div><Link to="/activate" className="rounded-xl bg-slate-950 px-4 py-3 text-xs font-black text-white">Vincular producto</Link></div>{message && <p className="mb-4 rounded-xl bg-cyan-50 px-3 py-2 text-sm font-bold text-cyan-700">{message}</p>}{loading ? <p className="text-slate-400">Cargando…</p> : <div className="grid gap-4 sm:grid-cols-2">{artifacts.map(item => <div key={item.id} className="rounded-[24px] border border-slate-200 bg-white p-5"><p className="text-xs font-black uppercase tracking-widest text-slate-400"><ProductLabel type={item.product_type} /></p><p className="mt-2 font-mono text-lg font-bold">{item.public_code}</p><p className="mt-2 text-sm text-slate-500">{item.profile_slug ? `Vinculado a /${item.profile_slug}` : 'Sin perfil vinculado'}</p>{!item.profile_id && me?.profile_id && <button onClick={() => linkProfile(item)} className="mt-4 text-sm font-black text-cyan-700">Vincular a mi perfil →</button>}{item.profile_slug && <a href={item.public_url} className="mt-4 block text-sm font-black text-cyan-700">Abrir enlace público →</a>}</div>)}</div>}</section></main>
}
...
# 2) AdminGuard resumes activation once a real profile exists.
guard = Path('app/src/components/admin/AdminGuard.tsx')
s = guard.read_text()
s = s.replace("import { useNavigate } from 'react-router-dom'", "import { useLocation, useNavigate } from 'react-router-dom'")
s = s.replace("  const navigate = useNavigate()\n", "  const navigate = useNavigate()\n  const location = useLocation()\n")
needle = "      const planId = json.data?.plan_id || json.data?.plan_code || 'free'\n"
insert = "      const pendingActivation = sessionStorage.getItem('intap_activation_public_code')\n      if (pendingActivation && json.data?.profile_id && location.pathname !== '/admin/artifacts/activate') {\n        navigate('/admin/artifacts/activate', { replace: true })\n        return\n      }\n\n"
if insert not in s:
    s = s.replace(needle, insert + needle)
s = s.replace("  }, [navigate, planScope, requireProfile])", "  }, [location.pathname, navigate, planScope, requireProfile])")
guard.write_text(s)

# 3) Magic-link callback resumes public-code-first activation from browser state.
callback = Path('app/src/components/admin/AuthCallback.tsx')
s = callback.read_text()
old = "          apiGet('/me/artifacts/activation/intent')\n            .then((intent: any) => navigate(intent.ok ? '/admin/artifacts/activate' : '/admin', { replace: true }))\n            .catch(() => navigate('/admin', { replace: true }))"
new = "          const pendingPublicCode = sessionStorage.getItem('intap_activation_public_code')\n          if (pendingPublicCode) {\n            navigate('/admin/artifacts/activate', { replace: true })\n            return\n          }\n          apiGet('/me/artifacts/activation/intent')\n            .then((intent: any) => navigate(intent.ok ? '/admin/artifacts/activate' : '/admin', { replace: true }))\n            .catch(() => navigate('/admin', { replace: true }))"
if old not in s:
    raise SystemExit('AuthCallback marker not found')
s = s.replace(old, new)
callback.write_text(s)

# 4) Onboarding done continues pending activation automatically from the CTA.
done = Path('app/src/components/admin/free/onboarding/FreeOnboardingDone.tsx')
s = done.read_text()
s = s.replace("          <button onClick={() => navigate('/admin/free')} className=", "          <button onClick={() => navigate(sessionStorage.getItem('intap_activation_public_code') ? '/admin/artifacts/activate' : '/admin/free')} className=")
s = s.replace('Ir a mi panel</button>', "{sessionStorage.getItem('intap_activation_public_code') ? 'Continuar activación' : 'Ir a mi panel'}</button>")
done.write_text(s)

# 5) Excel export: real .xlsx via SheetJS.
artifacts = Path('app/src/components/admin/SuperAdminArtifacts.tsx')
s = artifacts.read_text()
if "from 'xlsx'" not in s:
    s = s.replace("import SuperAdminLayout from './SuperAdminLayout'", "import SuperAdminLayout from './SuperAdminLayout'\nimport * as XLSX from 'xlsx'")
s = s.replace('Descarga el CSV antes de salir de la pantalla.', 'Descarga el archivo Excel antes de salir de la pantalla.')
s = s.replace('descarga sus códigos ahora.', 'descarga sus códigos en Excel ahora.')
start = s.find('  function downloadBatchCsv() {')
end = s.find('\n\n  async function copyValue', start)
if start == -1 or end == -1:
    raise SystemExit('CSV function markers not found')
excel_fn = r'''  function downloadBatchExcel() {
...
s = s[:start] + excel_fn + s[end:]
s = s.replace('downloadBatchCsv()', 'downloadBatchExcel()')
s = s.replace('Descargar CSV', 'Descargar Excel (.xlsx)')
s = s.replace('CSV', 'Excel')
artifacts.write_text(s)

# 6) Add xlsx dependency. npm install will update package-lock locally before commit/deploy.
pkg_path = Path('app/package.json')
pkg = json.loads(pkg_path.read_text())
pkg.setdefault('dependencies', {})['xlsx'] = '^0.18.5'
pkg_path.write_text(json.dumps(pkg, indent=4, ensure_ascii=False) + '\n')

print('Applied activation v2 UI, resume flow and real XLSX batch export.')
