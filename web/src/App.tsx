...
function RootRoute() {
  ...
  useEffect(() => { if (!slug) window.location.replace(KAWVO_LINK_HOME) }, [slug])
  if (slug) return <Navigate to={`/${slug}`} replace />
  return <RouteLoader />
}

function PublicProfileRoute() {
  return <><PublicProfile /><PublicBankAccounts /></>
}
    ...
    <BrowserRouter>
      ...
      <Suspense fallback={<RouteLoader />}>
        <Routes>
          ...
          <Route path="/demo/s/:token" element={<KawvoLinkDemoShared />} />
          ...
          <Route path="/:slug/bancos" element={<LegacyBankRoute />} />
