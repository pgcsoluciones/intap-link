...
app.post('/api/v1/public/team/member-access/login',async(c:any)=>{
  ...
  }catch(error){console.error('[team/member-access/login] failed',error);return c.json({ok:false,error:'No pudimos iniciar el acceso Team.'},500)}
})
...
app.post('/api/v1/me/team/access/change-password',requireAuth,async(c:any)=>{
  ...
  }catch(error){console.error('[team/access/change-password] failed',error);return c.json({ok:false,error:'No pudimos cambiar la contraseña Team.'},500)}
})
...
app.post('/api/v1/me/team/members/:id/role',requireAuth,async(c:any)=>{
  ...
  }catch(error){console.error('[team/members/role] failed',error);return c.json({ok:false,error:'No pudimos asignar el rol Team.'},500)}
})
