...
path = Path(__file__).resolve().parents[1] / 'api' / 'src' / 'index.ts'
source = path.read_text()
marker = '// ─── /me endpoints — sub-app aislado, requireAuth se aplica una sola vez ──'
sentinel = '// ─── Super Admin — physical artifacts management ─────────────────────────'

if sentinel in source:
    print('Super Admin artifact API already present; nothing to do.')
    raise SystemExit(0)
if marker not in source:
    raise SystemExit('Insertion marker not found in api/src/index.ts')
...
print('Inserted Super Admin artifact management API into api/src/index.ts')
