...
BRAND='app/src/kawvo-brand.ts'
BRAND_CSS='app/src/kawvo-brand.css'
...
check "$MW" 'data-kawvo-server-profile' 'El perfil semántico no se entrega como HTML inicial'
check "$MW" 'output.replace(emptyRoot' 'La semántica no se inserta dentro del root inicial'
check "$MW" '/ai.md' 'Falta señal AI Discovery en metadata dinámica'
check "$BRAND" "MARK_LOGO_PATH = '/assets/free-starter/branding/logo-solo.png'" 'Branding de pantalla no usa logo solo oficial'
check "$BRAND_CSS" '.kawvo-brand-logo-screen' 'Falta tamaño legible para branding de pantalla'
...
check "$SOCIAL_QA" 'data-kawvo-server-profile' 'QA social no valida HTML semántico inicial'
