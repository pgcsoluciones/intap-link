...
export default function SponsoredStarterOnboarding({onComplete}:{onComplete:()=>void|Promise<void>}){
  ...
  async function continueStarter(){
    if(!category||!username||saving)return
  ...
  return <section className="mx-auto w-full max-w-[760px] rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_18px_55px_rgba(15,23,42,.07)] sm:p-6">
    ...
    <h2 className="mt-2 text-2xl font-black tracking-[-.03em] text-slate-950">¿A qué te dedicas?</h2>
    <p className="mt-2 text-sm leading-6 text-slate-600">Con tu actividad y tu nombre de usuario prepararemos un borrador base con contenido e imágenes de ejemplo. Después solo tendrás que reemplazarlo por tus datos reales.</p>
    ...
    <div className="mt-6">
      <button type="button" onClick={()=>void continueStarter()} disabled={!category||username.length<3||saving} className="w-full rounded-2xl bg-slate-950 px-4 py-4 text-sm font-black text-white transition hover:bg-slate-800 disabled:opacity-35">{saving?'Preparando tu borrador…':'Crear mi borrador base'}</button>
