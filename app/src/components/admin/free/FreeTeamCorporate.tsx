...
export default function FreeTeamCorporate(){
  ...
  const load=async(nextPage=page,q=query)=>{
    ...
    if(admin.data?.access?.role==='master'){
      ...
      if(settings?.ok)setShowBankAccounts(settings.data?.show_bank_accounts!==false)
    }
    setLoading(false)
  }
