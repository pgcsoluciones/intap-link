...
BRANCH="feature/sponsor-code-list-gallery-batch-v1"
EXPECTED_MAIN_SHA="7be94abe8375cc8ffec4bb21b5f5d79930c4f127"
APPROVED_PRODUCT_SHA="56afd152dd100f9bf3f09e0df702b5cfb19850b9"
RUNNER_PATH="scripts/run-production-sponsor-code-list-gallery-batch-v1-2026-09-18.sh"
APP_PROJECT="intap-web2"
LOG_DIR="$ROOT/.production-sponsor-code-list-gallery-batch-v1-logs"
...
cat <<EOF
============================================================
KAWVO LINK · CÓDIGOS SUPERADMIN + GALERÍA POR LOTE · PRODUCCIÓN
============================================================
- listado persistente y paginado de códigos por patrocinador
- búsqueda y filtros por estado/tipo
- carga múltiple de galería patrocinada hasta 10 imágenes
- conserva carga individual con recorte
- NO aplica migraciones
- NO toca D1
- NO despliega Web pública
- despliega Worker + App
- contrato + builds + smoke antes de promover main
============================================================
Main esperado:  $EXPECTED_MAIN_SHA
Producto SHA:   $APPROVED_PRODUCT_SHA
============================================================
EOF
...
ALLOWED='^(api/src/sponsored-profiles\.ts|app/src/components/admin/SuperAdminSponsors\.tsx|app/src/components/admin/sponsored/SponsoredDashboard\.tsx|scripts/test-sponsored-profile-contract\.mjs|scripts/run-preview-superadmin-sponsor-code-list-v1-2026-09-18\.sh|scripts/run-production-sponsor-code-list-gallery-batch-v1-2026-09-18\.sh)$'
...
run npx esbuild api/src/preview-free-entry.ts --bundle --platform=node --format=esm --target=node20 --external:cloudflare:* --outfile=/tmp/kawvo-sponsored-code-list-gallery-production-api.mjs
...
TAG="prod-sponsor-code-list-gallery-batch-v1-2026-09-18-$(date +%H%M%S)"
run git tag -a "$TAG" -m "Sponsor code list and gallery batch V1 production 2026-09-18"
...
cat <<EOF
============================================================
✓ CÓDIGOS + GALERÍA POR LOTE DESPLEGADOS EN PRODUCCIÓN
============================================================
