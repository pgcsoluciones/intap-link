...
export default function KawvoTrial({mode}:{mode:Mode}){
  ...
  const editorTop=(mode==='editor'||mode==='owner')?<div className="trial-adminbar"><div className="trial-adminbar-main"><strong>{mode==='owner'?'Editando presentación':'TRIAL · Editando'}</strong><button className="trial-tools-menu" aria-label="Opciones de edición" aria-expanded={toolsOpen} onClick={()=>{setToolsOpen(v=>!v);setExtrasOpen(false)}}><FaBars/><span>Opciones</span></button><div className={`trial-tools ${toolsOpen?'open':''}`}><button className="trial-tool" onClick={()=>{openSection("appearance");setToolsOpen(false);setExtrasOpen(false)}}><FaPalette/> Colores</button><div className="trial-extras-wrap"><button className="trial-tool trial-extras-trigger" onClick={()=>setExtrasOpen(v=>!v)}><FaEllipsisH/> Extras</button>{extrasOpen&&<div className="trial-extras-menu"><button onClick={()=>{openSection("links");setExtrasOpen(false);setToolsOpen(false)}}><FaPlus/> Mis enlaces</button><button onClick={()=>{openSection("banks");setExtrasOpen(false);setToolsOpen(false)}}><FaUniversity/> Cuentas bancarias</button></div>}</div>{mode==='editor'&&<button className="trial-tool" onClick={()=>{void openProspect();setToolsOpen(false);setExtrasOpen(false)}}><FaPlus/> Prospecto</button>}</div>{mode==='owner'&&<div className="trial-owner-actions"><button className="trial-owner-bell" aria-label="Notificaciones" onClick={()=>setNotificationsOpen(v=>!v)}><FaBell/>{unreadNotifications>0&&<b>{unreadNotifications}</b>}</button></div>}<button className="trial-primary trial-finish-btn" onClick={()=>{setTrialName(snapshot.profile.name);setTrialSlug(trial?.slug||suggestedSlug(snapshot.profile.name));setFinish(true);setExtrasOpen(false)}}><FaCheck/><span>Finalizar</span></button><span className={`trial-save ${saving}`}>{saving==='saving'?'Guardando…':saving==='error'?'Error':'Guardado'}</span></div></div>:undefined
  ...
  return <>
    ...
    {copiedMessage&&<div className="trial-copy-toast">{copiedMessage}</div>}
