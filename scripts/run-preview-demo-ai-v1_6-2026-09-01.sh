...
LOG_DIR="$ROOT/.preview-demo-ai-v1_6-2026-09-01-logs"
...
run python3 scripts/apply-demo-ai-product-refinements-v1_6.py
run git diff --check
...
run git add \
  ...
  scripts/apply-demo-ai-product-refinements-v1_6.py \
  scripts/test-demo-ai-product-v1_6.mjs \
  scripts/run-preview-demo-ai-v1_6-2026-09-01.sh

if ! git diff --cached --quiet; then
  run git commit -m "feat(demo): disable share controls during demo"
...
if ! git diff --cached --quiet; then
  run git commit -m "chore(preview): pin Demo AI v1.6 web origin"
  run git push github "HEAD:$BRANCH"
fi
...
grep -R "Mostrar cómo se verían tus datos para recibir transferencias" web/dist/assets >/dev/null || fail "CTA bancario nuevo no llegó al build"
grep -R "ilx-share button" web/dist/assets >/dev/null || fail "Bloqueo demo de compartir no llegó al build"

echo "✓ refinamientos V1.6 presentes en bundle Preview"

cat <<EOF

============================================================
✓ KAWVO LINK · DEMO IA V1.6 · PREVIEW LISTO PARA QA VISUAL
============================================================
...
Incluido:
- Compartir, Copiar enlace y Código QR permanecen visibles, pero desactivados durante Demo;
- el bloqueo aplica tanto a la Demo propia como a la Demo compartida;
- CTA bancario: "Mostrar cómo se verían tus datos para recibir transferencias";
- QA anterior reejecutado.
============================================================
EOF
