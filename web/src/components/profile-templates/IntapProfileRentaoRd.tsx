...
const RENTAO = {
  ...
  hero: '/assets/rentaord/hero%202_.png',
...
const VEHICLES: RentaoVehicle[] = [
  ...
  },
  {
    brand: 'Kia',
    model: 'Río 2023',
    image: '/assets/rentaord/Kia-Río.png',
    description:
...
const MARBELLA = {
  ...
  pdf: `${(import.meta.env.VITE_API_URL || '').replace(/\/$/, '')}/api/v1/public/assets/rentaord/MARBELLA-BOAT.pdf`,
  images: [
...
export default function IntapProfileRentaoRd({ profile }: { profile?: any }) {
  ...
  useEffect(() => {
    ...
    return () => {
      window.clearInterval(marbellaTimer)
    }
  }, [])
  ...
  return (
    <div className="rentaord-page">
      ...
      <div className="rentaord-shell">
        ...
        <section className="rentaord-identity">
          ...
          <div className="rentaord-hero-actions">
            <a
              className="rentaord-btn rentaord-btn--primary"
              href={getWhatsAppHref(getGeneralAvailabilityMessage())}
              target="_blank"
              rel="noopener noreferrer"
            >
              Reservar por WhatsApp
            </a>
            <a className="rentaord-btn rentaord-btn--secondary" href="#vehiculos">
        ...
        <section className="rentaord-about">
          ...
          <p>
            RENTAO RD es una empresa de rent car en República Dominicana enfocada en ofrecer soluciones
            de movilidad seguras, confiables y sin complicaciones.
          </p>
          <p>
            Nos especializamos en vehículos cómodos, modernos y listos para que cada cliente pueda
            desplazarse con tranquilidad. Trabajamos con unidades previamente inspeccionadas y en
            óptimas condiciones, brindando un servicio cercano, transparente y personalizado.
          </p>
        </section>
        ...
        <section className="rentaord-section" id="vehiculos">
          <div className="rentaord-section-head">
            ...
            <p>
              Elige el vehículo según tu necesidad: opciones compactas para moverte ágil, SUVs para viajar cómodo y modelos amplios para familia, trabajo o presencia ejecutiva. Explora nuestro catálogo completo de modelos y consulta disponibilidad al instante por WhatsApp.
            </p>
          </div>
          ...
          <div className="rentaord-showcase-grid">
            {visibleVehicles.map((vehicle, index) => (
              <button
                ...
                type="button"
                className="rentaord-feature-card"
                onClick={() => setGalleryOpen(true)}
              >
                <div className="rentaord-feature-media">
          ...
          <button
            type="button"
            className="rentaord-all-models-btn"
            onClick={() => setGalleryOpen(true)}
          >
        ...
        <section className="rentaord-section rentaord-marbella-section" id="marbella">
          ...
          <article className="rentaord-marbella-card">
            <button
              type="button"
              className="rentaord-marbella-slider"
              aria-label="Ver detalles de Lancha Marbella"
              onClick={() => setMarbellaOpen(true)}
            >
              {MARBELLA.images.map((image, index) => (
                <img
                  key={image}
                  className={`rentaord-marbella-slide${index === marbellaSlideIndex ? ' is-active' : ''}`}
                  src={image}
                  alt={`Marbella Boat vista ${index + 1}`}
                />
              ))}
          ...
          <div className="rentaord-benefits-grid">
            {BENEFITS.map((benefit) => (
              <article key={benefit.title}>
                <div className="rentaord-benefit-icon">
        ...
        <section className="rentaord-contact" id="contacto">
          <div className="rentaord-section-head">
            ...
            <p>Comunícate por el canal que prefieras para reservar, consultar disponibilidad o recibir orientación rápida.</p>
          </div>

          <div className="rentaord-contact-list">
            <a href={`tel:+1${RENTAO.phonePrimary.replace(/\D/g, '')}`} className="rentaord-contact-row">
              <FaPhoneAlt aria-hidden="true" />
              <strong>{RENTAO.phonePrimary}</strong>
            </a>
        ...
        <section className="rentaord-contact-cta">
          ...
          <button type="button" className="rentaord-btn rentaord-btn--primary" onClick={() => setContactOpen(true)}>
            Contáctanos
          </button>
        </section>
      ...
      {marbellaOpen ? (
        <div className="rentaord-modal-backdrop" onClick={() => setMarbellaOpen(false)}>
          <div className="rentaord-modal rentaord-marbella-modal" onClick={(event) => event.stopPropagation()}>
            <button
              type="button"
              className="rentaord-modal-close"
              aria-label="Cerrar detalles de Marbella"
              onClick={() => {
                setMarbellaOpen(false)
                setMarbellaZoomIndex(null)
              }}
            >
              ×
            </button>
            ...
            <div className="rentaord-marbella-modal-head">
              ...
              <div className="rentaord-marbella-pills">
                ...
                <span>2 tripulantes</span>
                <span>Sonido premium</span>
              </div>
            </div>
            ...
            <div className="rentaord-marbella-gallery">
              {MARBELLA.images.map((image, index) => (
                <button
                  type="button"
                  key={image}
                  className="rentaord-marbella-gallery-item"
                  aria-label={`Ampliar foto ${index + 1} de Marbella Boat`}
                  onClick={() => setMarbellaZoomIndex(index)}
                >
            ...
            <div className="rentaord-marbella-detail-grid">
              ...
              <section>
                <h4>Permitido</h4>
                <ul>
                  {MARBELLA.allowed.map((item) => (
                    <li key={item}>{item}</li>
                  ))}
                </ul>
              </section>

              <section className="rentaord-marbella-wide">
                <h4>No permitido</h4>
                <ul>
                  {MARBELLA.notAllowed.map((item) => (
                    <li key={item}>{item}</li>
                  ))}
                </ul>
              </section>
            ...
            <div className="rentaord-marbella-actions">
              <a
                className="rentaord-btn rentaord-btn--primary"
                href={getWhatsAppHref('Hola, vi la experiencia Marbella Boat y quiero consultar disponibilidad, rutas y precios.')}
                target="_blank"
                rel="noopener noreferrer"
              >
      ...
      {marbellaZoomIndex !== null ? (
        <div className="rentaord-marbella-zoom" onClick={() => setMarbellaZoomIndex(null)}>
          <div className="rentaord-marbella-zoom-stage" onClick={(event) => event.stopPropagation()}>
            <button
              type="button"
              className="rentaord-marbella-zoom-close"
              aria-label="Cerrar imagen ampliada"
              onClick={() => setMarbellaZoomIndex(null)}
            >
              ×
            </button>
            ...
            <button
              type="button"
              className="rentaord-marbella-zoom-nav rentaord-marbella-zoom-nav--prev"
              aria-label="Imagen anterior"
              onClick={() =>
                setMarbellaZoomIndex((current) =>
                  current === null ? current : (current - 1 + MARBELLA.images.length) % MARBELLA.images.length,
                )
              }
            >
              ‹
            ...
            <img
              src={MARBELLA.images[marbellaZoomIndex]}
              alt={`Marbella Boat imagen ampliada ${marbellaZoomIndex + 1}`}
            />

            <button
              type="button"
              className="rentaord-marbella-zoom-nav rentaord-marbella-zoom-nav--next"
              aria-label="Imagen siguiente"
              onClick={() =>
                setMarbellaZoomIndex((current) =>
                  current === null ? current : (current + 1) % MARBELLA.images.length,
                )
              }
            >
              ›
            ...
            <div className="rentaord-marbella-zoom-count">
              {marbellaZoomIndex + 1} / {MARBELLA.images.length}
            </div>
          </div>
        </div>
      ) : null}
          ...
          <div className="rentaord-modal rentaord-contact-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <label>
              ...
              <textarea
                ...
                placeholder="Ej.: Necesito una SUV para fin de semana, servicio ejecutivo o renta familiar..."
              />
            </label>

            <a
              className="rentaord-btn rentaord-btn--primary"
              href={getWhatsAppHref(getContactFormMessage(leadName, leadPhone, leadNeed))}
              target="_blank"
              rel="noopener noreferrer"
            >
              <FaWhatsapp aria-hidden="true" />
      ...
      <div className={`rentaord-chat ${chatOpen ? 'is-open' : ''}`}>
        {chatOpen ? (
          <div className="rentaord-chat-panel">
            <div className="rentaord-chat-head">
              ...
              <button type="button" aria-label="Cerrar chat" onClick={() => setChatOpen(false)}>
            ...
            <div className="rentaord-chat-body">
              ...
              <a href={getWhatsAppHref(getChatMessage('Quiero consultar disponibilidad de un vehículo.'))} target="_blank" rel="noopener noreferrer">
                Consultar disponibilidad
              </a>
              <a href={getWhatsAppHref(getChatMessage('Quiero saber qué modelos tienen disponibles.'))} target="_blank" rel="noopener noreferrer">
                Ver modelos disponibles
              </a>
              <a href={getWhatsAppHref(getChatMessage('Necesito un vehículo ejecutivo.'))} target="_blank" rel="noopener noreferrer">
                Necesito servicio ejecutivo
              </a>
              <a href={getWhatsAppHref(getChatMessage('Quiero recibir asistencia personalizada.'))} target="_blank" rel="noopener noreferrer">
        ...
        <button type="button" className="rentaord-chat-bubble" aria-label="Abrir chat Rentao RD" onClick={() => setChatOpen((current) => !current)}>
        ...
        <div className="rentaord-modal-backdrop" onClick={() => setGalleryOpen(false)}>
          <div className="rentaord-modal rentaord-modal--catalog" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="rentaord-modal-head">
              ...
              <p>
                Aquí puedes ver todos los modelos. Desde cada uno puedes abrir su ficha o consultar disponibilidad.
              </p>
            </div>
            ...
            <div className="rentaord-gallery-grid">
              {VEHICLES.map((vehicle) => (
                <article
                  ...
                  className="rentaord-gallery-card"
                  role="button"
                  tabIndex={0}
                ...
                >
                  <div className="rentaord-gallery-media">
                    <img src={vehicle.image} alt={`${vehicle.brand} ${vehicle.model}`} />
                  </div>

                  <div className="rentaord-gallery-copy">
                    <span>{vehicle.brand}</span>
                    <h4>{vehicle.model}</h4>
                  </div>
                  ...
                  <div className="rentaord-gallery-actions">
                    <button
                      type="button"
                      className="rentaord-btn rentaord-btn--secondary rentaord-btn--details"
                      onClick={(event) => {
                    ...
                    <a
                      className="rentaord-btn rentaord-btn--primary"
                      href={getWhatsAppHref(getVehicleAvailabilityMessage(vehicle))}
                      target="_blank"
                      rel="noopener noreferrer"
      ...
      {selectedVehicle ? (
        <div className="rentaord-modal-backdrop" onClick={() => setSelectedVehicle(null)}>
          <div className="rentaord-modal rentaord-modal--detail" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="rentaord-detail-block">
              <h4>Beneficios</h4>
              <ul>
                {selectedVehicle.benefits.map((item) => (
                  <li key={item}>{item}</li>
                ))}
              </ul>
            </div>
            ...
            <div className="rentaord-detail-actions">
              <a
                className="rentaord-btn rentaord-btn--primary"
                href={getWhatsAppHref(getVehicleAvailabilityMessage(selectedVehicle))}
                target="_blank"
                rel="noopener noreferrer"
              >
                Consultar disponibilidad
