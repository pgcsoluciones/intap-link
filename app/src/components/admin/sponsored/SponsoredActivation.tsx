...
export default function SponsoredActivation(){
  ...
  return <main className="min-h-screen bg-[#f7f9fc] px-5 py-8 font-['Inter'] text-slate-950"><section className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-[460px] flex-col justify-center"><div className="rounded-[30px] border border-slate-200 bg-white p-6 text-center shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
    ...
    {!error&&<div className="mt-5 rounded-[22px] border border-cyan-100 bg-cyan-50/60 p-4 text-left"><p className="text-sm font-black text-slate-800">Antes de activar, queremos que lo tengas claro</p><p className="mt-2 text-[13px] leading-6 text-slate-600">Esta presentación digital es impulsada por <strong>{sponsorName}</strong> como una contribución a la digitalización y modernización de sus clientes y aliados.</p><p className="mt-2 text-[13px] leading-6 text-slate-600">Como parte de este patrocinio, al final de tu presentación aparecerá un <strong>cintillo discreto</strong> identificando a {sponsorName} como la marca que impulsa esta iniciativa. Su finalidad es acompañar y fortalecer tu presencia digital, no competir con tu negocio.</p><p className="mt-2 text-[13px] leading-6 text-slate-600"><strong>Tu presentación y tus datos son tuyos.</strong> {sponsorName} no podrá entrar a tu cuenta, editar tu información ni acceder a tus credenciales o datos privados. Solo podrá consultar la trazabilidad básica del programa, como el estado de activación del producto.</p><label className="mt-4 flex cursor-pointer items-start gap-3 rounded-2xl bg-white p-3"><input type="checkbox" checked={accepted} onChange={e=>setAccepted(e.target.checked)} className="mt-1 h-4 w-4 shrink-0"/><span className="text-[13px] leading-5 text-slate-700"><strong>Estoy de acuerdo</strong> con estas condiciones de patrocinio y deseo activar mi presentación digital.</span></label></div>}
