...
const HELP: Array<{ match: (path: string) => boolean; entry: HelpEntry }> = [
  ...
  { match: (p) => p === '/admin/free/account', entry: { title: 'Mi cuenta', intro: 'Aquí concentras la gestión de tu cuenta Kawvo.', points: ['Revisa tu plan y tus cuotas de IA.', 'Consulta notificaciones, productos y tickets de ayuda.', 'Administra sesiones activas y opciones de tu perfil.'] } },
...
export default function FreeContextHelp() {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-[90] font-['Inter']">
      {open && (
        <section className="mb-3 w-[min(350px,calc(100vw-32px))] rounded-[24px] border border-slate-200 bg-white p-5 shadow-2xl" role="dialog" aria-label={`Ayuda: ${help.title}`}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.14em] text-cyan-700">Ayuda rápida</p>
              <h2 className="mt-1 text-lg font-black text-slate-950">{help.title}</h2>
            </div>
            <button type="button" onClick={() => setOpen(false)} className="rounded-full bg-slate-100 px-3 py-2 text-base font-black text-slate-700">×</button>
          </div>
          <p className="mt-3 text-base font-medium leading-7 text-slate-700">{help.intro}</p>
          <ul className="mt-4 space-y-3 text-base font-medium leading-7 text-slate-700">
            {help.points.map((point) => <li key={point} className="flex gap-2.5"><span className="font-black text-cyan-700">✓</span><span>{point}</span></li>)}
          </ul>
        </section>
      )}
      <button type="button" onClick={() => setOpen((current) => !current)} className="ml-auto flex h-13 items-center gap-2 rounded-full bg-slate-950 px-4 text-base font-black text-white shadow-xl" aria-expanded={open}>
        <span className="flex h-7 w-7 items-center justify-center rounded-full bg-cyan-500">?</span>
        <span>Ayuda</span>
      </button>
    </div>
  )
