...
function seed(db, { codeExpiresAt = '2099-01-01 00:00:00.000', intentExpiresAt = codeExpiresAt, secondIntent = false } = {}) {
  ...
  if (secondIntent) db.prepare("INSERT INTO artifact_activation_intents (id, intent_hash, artifact_id, activation_code_id, status, expires_at) VALUES ('intent-2', 'intent-hash-2', 'artifact-1', 'code-1', 'active', '2099-01-01 00:00:00.000')").run()
}
...
function state(db) {
  return {
    ...
    receipt: (() => { const row = db.prepare('SELECT intent_hash, user_id, profile_id, claim_at, ok FROM artifact_activation_claims ORDER BY intent_hash LIMIT 1').get(); return row ? { ...row } : row })(),
  }
}
...
function claimStatements({ intentHash, userId, profileId = null, claimAt = CLAIM_AT, codeId = 'code-1' }) {
  return [
    ...
    [
      `INSERT INTO artifact_activation_claims (intent_hash, user_id, profile_id, claim_at, ok)
      ...
      [intentHash, userId, profileId, claimAt, intentHash, claimAt, claimAt, userId, claimAt, profileId, profileId, profileId, profileId, userId],
    ],
  ]
}
...
for (const preview of [false, true]) {
  ...
  assert.match(ddl, /intent_hash TEXT PRIMARY KEY/)
...
for (const preview of [false, true]) {
  ...
  assert.deepEqual(s.receipt, { intent_hash: 'intent-hash-1', user_id: 'user-1', profile_id: null, claim_at: CLAIM_AT, ok: 1 })
...
const workerCode = `
  ...
  const q = [
    ...
    [\`INSERT INTO artifact_activation_claims (intent_hash, user_id, profile_id, claim_at, ok) VALUES (?, ?, NULL, ?, CASE WHEN EXISTS (SELECT 1 FROM artifact_activation_intents i JOIN artifact_activation_codes ac ON ac.id = i.activation_code_id JOIN intap_artifacts a ON a.id = i.artifact_id AND ac.artifact_id = a.id WHERE i.intent_hash = ? AND i.status = 'consumed' AND i.consumed_at = ? AND ac.status = 'used' AND ac.used_at = ? AND a.owner_user_id = ? AND a.status = 'activated' AND a.activated_at = ?) THEN 1 ELSE 0 END)\`, [h, u, at, h, at, at, u, at]],
  ]
...
console.log('B2B.3A SQLite atomic receipt tests: PASS (persistent one-time receipt, same-claimAt retry, assertion rollback, expiry, revocation, concurrency)')
