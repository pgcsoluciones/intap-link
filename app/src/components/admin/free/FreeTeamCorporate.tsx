...
type ManagePayload = { access:{ role:'master'|'editor'|'subadmin'; can_generate_codes:boolean; can_manage_roles:boolean; can_toggle_members:boolean; can_edit_members:boolean }; members:MemberRow[]; member_pagination?:Pagination }
...
export default function FreeTeamCorporate(){
  ...
  const load=async(nextPage=page,q=query,nextMemberPage=memberPage)=>{
    ...
    setBasic(base.data);setManage(admin.data);setPage(base.data?.pagination?.page||nextPage);setMemberPage(admin.data?.member_pagination?.page||nextMemberPage)
    ...
    if(admin.data?.access?.role==='master'){
  ...
  const saveName=async()=>{
    ...
    setSavedName(json.data?.name||name);setTeamName(json.data?.name||name);setNameConfirmed(true);setMessage('Nombre interno del Team guardado.')
  }
  const saveCompanyName=async()=>{
    ...
    setMessage('Nombre de empresa guardado y sincronizado con los perfiles Team.')
  }
  const saveBankVisibility=async(next:boolean)=>{
    ...
    setMessage(next?'Las cuentas bancarias del Master se mostrarán en los perfiles vinculados.':'Las cuentas bancarias quedaron ocultas en los perfiles vinculados.')
  }
  ...
  const createCodes=async()=>{
    ...
    const codes=(json.data||[]).map((item:any)=>String(item.code||''));setGenerated(codes);setGeneratedPage(1);setMessage(`${codes.length} código(s) creado(s). Estado inicial: Sin asignar.`);await load(1,query,memberPage)
  }
  const codeAction=async(row:CodeRow,type:'deactivate'|'reactivate')=>{
    ...
    setMessage(type==='reactivate'?'Código reactivado y disponible por 24 horas.':'Código desactivado.');await load(page,query,memberPage)
  }
  ...
  const toggleMember=async(member:MemberRow)=>{
    ...
    setMessage(active?'Miembro activado.':'Miembro desactivado. El dispositivo mostrará Perfil no disponible.');await load(page,query,memberPage)
  }
