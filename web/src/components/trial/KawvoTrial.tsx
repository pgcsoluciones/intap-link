...
export default function KawvoTrial({mode}:{mode:Mode}){
  ...
  return <>
    ...
    {panel&&section&&<div className="trial-sheet-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setPanel(false)}}><section className="trial-sheet trial-context-sheet">
      ...
      {section==='portfolio'&&<><div className="trial-gallery-head"><strong>Imágenes</strong><span className="trial-upload"><FaCamera/> Agregar<input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" onChange={e=>chooseImage('gallery',e.target.files?.[0])}/></span></div>{snapshot.profile.portfolio.map((item,index)=><div className="trial-work" key={item.id}><div className="trial-replace-wrap"><img src={item.image} alt=""/><label className="trial-replace">Reemplazar<input type="file" accept="image/*" onChange={e=>{const f=e.target.files?.[0];if(!f)return;setCrop({kind:'gallery',file:f});setCropIndex(index)}}/></label></div><div><input value={item.title} onChange={e=>change('portfolio',snapshot.profile.portfolio.map((z,i)=>i===index?{...z,title:e.target.value}:z))}/><textarea rows={2} value={item.description} onChange={e=>change('portfolio',snapshot.profile.portfolio.map((z,i)=>i===index?{...z,description:e.target.value}:z))}/></div><button onClick={()=>removePortfolio(item.id)}>Eliminar</button></div>)}</>}
    </section></div>}
