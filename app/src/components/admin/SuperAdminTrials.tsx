      ...
      {selected&&prospect&&<section className="rounded-2xl border border-slate-300 bg-white p-5 shadow-sm">
        ...
        <div className="mt-7 grid gap-5 lg:grid-cols-2">
          <div className="rounded-2xl bg-slate-50 p-4">
            ...
            {selected.activated_at?<div className="mt-4 flex flex-wrap gap-2"><select className={field+' max-w-48'} value={extendHours} onChange={e=>setExtendHours(Number(e.target.value))}><option value={24}>+24 horas</option><option value={48}>+48 horas</option><option value={72}>+72 horas</option><option value={120}>+5 días</option><option value={168}>+7 días</option></select><button onClick={()=>void extendTrial()} disabled={saving} className="rounded-xl bg-cyan-700 px-4 py-2.5 text-sm font-black text-white">Extender Trial</button></div>:<p className="mt-3 text-sm font-bold text-amber-700">La duración comienza al publicar.</p>}
          </div>
          <div className="rounded-2xl bg-slate-50 p-4">
