...
export default function SponsoredExperienceTools(){
  ...
  async function openAccount(){
    ...
    const json:any=await apiGet(`/me/sponsored-profile/account${profileQuery}`).catch(()=>({ok:false}))
    if(json?.ok)setAccountEmail(String(json.data?.email||''))
    const list:any=await apiGet('/me/sponsored-profiles').catch(()=>({ok:false}))
    setProfiles(list?.ok&&Array.isArray(list.data?.profiles)?list.data.profiles:[])
  }
  async function unlink(){
    if(unlinking||!ack||phrase.trim().toUpperCase()!=='DESVINCULAR'||!emailConfirm.trim())return
    setUnlinking(true);setUnlinkError('')
    try{
      const json:any=await apiPost(`/me/sponsored-profile/unlink${profileQuery}`,{confirm_phrase:phrase,confirm_email:emailConfirm.trim()})
