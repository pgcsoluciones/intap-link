...
BEGIN
  ...
  SELECT CASE WHEN (
      ...
      NEW.free_brand_color IS NOT OLD.free_brand_color OR
      NEW.hero_url IS NOT OLD.hero_url OR
      NEW.hero_position_x IS NOT OLD.hero_position_x OR
      NEW.hero_position_y IS NOT OLD.hero_position_y OR
      NEW.hero_zoom IS NOT OLD.hero_zoom
    ) AND NOT EXISTS (
      SELECT 1 FROM team_members tm, json_each(tm.permissions_json) p
       WHERE tm.profile_id = OLD.id AND p.value = 'design'
    ) THEN RAISE(ABORT, 'team_permission_denied:design') END;
...
WHEN EXISTS (SELECT 1 FROM team_members tm WHERE tm.profile_id=NEW.profile_id)
 AND NOT EXISTS (SELECT 1 FROM team_members tm, json_each(tm.permissions_json) p WHERE tm.profile_id=NEW.profile_id AND p.value='portfolio')
BEGIN SELECT RAISE(ABORT, 'team_permission_denied:portfolio'); END;

CREATE TRIGGER IF NOT EXISTS trg_team_gallery_update_guard
BEFORE UPDATE ON profile_gallery
WHEN EXISTS (SELECT 1 FROM team_members tm WHERE tm.profile_id=OLD.profile_id)
 AND NOT EXISTS (SELECT 1 FROM team_members tm, json_each(tm.permissions_json) p WHERE tm.profile_id=OLD.profile_id AND p.value='portfolio')
BEGIN SELECT RAISE(ABORT, 'team_permission_denied:portfolio'); END;

CREATE TRIGGER IF NOT EXISTS trg_team_gallery_delete_guard
BEFORE DELETE ON profile_gallery
WHEN EXISTS (SELECT 1 FROM team_members tm WHERE tm.profile_id=OLD.profile_id)
 AND NOT EXISTS (SELECT 1 FROM team_members tm, json_each(tm.permissions_json) p WHERE tm.profile_id=OLD.profile_id AND p.value='portfolio')
BEGIN SELECT RAISE(ABORT, 'team_permission_denied:portfolio'); END;

CREATE TRIGGER IF NOT EXISTS trg_team_products_insert_guard
BEFORE INSERT ON profile_products
WHEN EXISTS (SELECT 1 FROM team_members tm WHERE tm.profile_id=NEW.profile_id)
 AND NOT EXISTS (SELECT 1 FROM team_members tm, json_each(tm.permissions_json) p WHERE tm.profile_id=NEW.profile_id AND p.value='services')
BEGIN SELECT RAISE(ABORT, 'team_permission_denied:services'); END;

CREATE TRIGGER IF NOT EXISTS trg_team_products_update_guard
BEFORE UPDATE ON profile_products
WHEN EXISTS (SELECT 1 FROM team_members tm WHERE tm.profile_id=OLD.profile_id)
 AND NOT EXISTS (SELECT 1 FROM team_members tm, json_each(tm.permissions_json) p WHERE tm.profile_id=OLD.profile_id AND p.value='services')
BEGIN SELECT RAISE(ABORT, 'team_permission_denied:services'); END;

CREATE TRIGGER IF NOT EXISTS trg_team_products_delete_guard
BEFORE DELETE ON profile_products
WHEN EXISTS (SELECT 1 FROM team_members tm WHERE tm.profile_id=OLD.profile_id)
 AND NOT EXISTS (SELECT 1 FROM team_members tm, json_each(tm.permissions_json) p WHERE tm.profile_id=OLD.profile_id AND p.value='services')
BEGIN SELECT RAISE(ABORT, 'team_permission_denied:services'); END;

CREATE TRIGGER IF NOT EXISTS trg_team_links_insert_guard
BEFORE INSERT ON profile_links
WHEN EXISTS (SELECT 1 FROM team_members tm WHERE tm.profile_id=NEW.profile_id)
 AND NOT EXISTS (SELECT 1 FROM team_members tm, json_each(tm.permissions_json) p WHERE tm.profile_id=NEW.profile_id AND p.value='links')
BEGIN SELECT RAISE(ABORT, 'team_permission_denied:links'); END;

CREATE TRIGGER IF NOT EXISTS trg_team_links_update_guard
BEFORE UPDATE ON profile_links
WHEN EXISTS (SELECT 1 FROM team_members tm WHERE tm.profile_id=OLD.profile_id)
 AND NOT EXISTS (SELECT 1 FROM team_members tm, json_each(tm.permissions_json) p WHERE tm.profile_id=OLD.profile_id AND p.value='links')
BEGIN SELECT RAISE(ABORT, 'team_permission_denied:links'); END;

CREATE TRIGGER IF NOT EXISTS trg_team_links_delete_guard
BEFORE DELETE ON profile_links
WHEN EXISTS (SELECT 1 FROM team_members tm WHERE tm.profile_id=OLD.profile_id)
 AND NOT EXISTS (SELECT 1 FROM team_members tm, json_each(tm.permissions_json) p WHERE tm.profile_id=OLD.profile_id AND p.value='links')
BEGIN SELECT RAISE(ABORT, 'team_permission_denied:links'); END;

CREATE TRIGGER IF NOT EXISTS trg_team_social_insert_guard
BEFORE INSERT ON profile_social_links
WHEN EXISTS (SELECT 1 FROM team_members tm WHERE tm.profile_id=NEW.profile_id)
 AND NOT EXISTS (SELECT 1 FROM team_members tm, json_each(tm.permissions_json) p WHERE tm.profile_id=NEW.profile_id AND p.value='quick_actions')
BEGIN SELECT RAISE(ABORT, 'team_permission_denied:quick_actions'); END;

CREATE TRIGGER IF NOT EXISTS trg_team_social_update_guard
BEFORE UPDATE ON profile_social_links
WHEN EXISTS (SELECT 1 FROM team_members tm WHERE tm.profile_id=OLD.profile_id)
 AND NOT EXISTS (SELECT 1 FROM team_members tm, json_each(tm.permissions_json) p WHERE tm.profile_id=OLD.profile_id AND p.value='quick_actions')
BEGIN SELECT RAISE(ABORT, 'team_permission_denied:quick_actions'); END;

CREATE TRIGGER IF NOT EXISTS trg_team_social_delete_guard
BEFORE DELETE ON profile_social_links
WHEN EXISTS (SELECT 1 FROM team_members tm WHERE tm.profile_id=OLD.profile_id)
 AND NOT EXISTS (SELECT 1 FROM team_members tm, json_each(tm.permissions_json) p WHERE tm.profile_id=OLD.profile_id AND p.value='quick_actions')
BEGIN SELECT RAISE(ABORT, 'team_permission_denied:quick_actions'); END;
